...
\begin{document}
    ...
    {Give a force along the line of two charges. \\ If \(q_1\) and \(q_2\) have the same sign (i.e. charge) then the force must be repulsive. But if they have the \textit{same} sign, then it must be attractive.\\ 
...
\[F_{3,net} = F_{31} + F_{32} \]
\[\vec{F_{31} = k \frac{q_3q_1}{r_{31}^2} \hat{r}_{31}}\]
\[\vec{F_{32} = k \frac{q_3q_2}{r_{32}^2} \hat{r}_{32}}\]
\[\qquad \]
\end{example}
